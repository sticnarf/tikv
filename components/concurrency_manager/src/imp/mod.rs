...
impl<M: ConcurrentMap> ConcurrencyManagerImpl<M> {
...
mod tests {
    ...
    async fn test_exclusive_lock() {
        let manager: Arc<ConcurrencyManagerImpl<Mutex<BTreeMap<Vec<u8>, KeyLock>>>> =
            Arc::new(ConcurrencyManagerImpl::new(1.into()));
    ...
    #[test]
    fn test_max_read_ts() {
        let manager: ConcurrencyManagerImpl<Mutex<BTreeMap<Vec<u8>, KeyLock>>> =
            ConcurrencyManagerImpl::new(1.into());
